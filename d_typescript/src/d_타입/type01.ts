...
//>> 변수명 뒤에 콜론을 사용하여 js코드에 타입을 정의 

//? 타입의 종류
//# 1. 기본 타입(원시타입-string,number,boolean 등)
// : 변수명 뒤에 콜론 사용
// >> tpyeof 연산자 사용 시 나타내는 변환값과 동일한 이름으로 명시(소문자)
...
let antData = '문자열'  //타입 명시 생략 가능 >> 타입 체킹을 생략x

// name = 123; -error 타입 체킹

// antData= 123; -error -타입을 명시하지 않아도 초기 할당하는 데이터로 타입 지정

//# 2. 배열(list,array)타입
// :순서가 있는 요소의 모음을 나타내는 자료 구조

//- 변수명 뒤에 콜론 사용
// :기본타입명 뒤에 배열을 나타내는 []을 첨부
...
function voidType(parameter:number):void{
  //함수의 타입 정의
  // >> 파라미터(매개변수)와 반환값(리턴값) 정의 가능
  // >> ts에서"파라미터에"타입을 명시하지 않으면 오류 발생
...
// >>사전에 오류 타입을 계산할 수 x(에러 방지x -사용 권장x)
...
// >> 타입을 직접적으로 명시하지 않을경우 값이 할당되기 전까지 자동으로 any타입으로 인식

//# 6.never 타입
// :절대 발생하지 않는 값의 타입
// >> 함수가 예외를 발생시키거나 끝나지 않을 때 사용

function error(meseage:string):never {
  throw new Error(meseage);
};
// error('error발생!');  -error
