...
let str = generic<string>('안녕하세요');   // 함수 호출 시 타입 변수에 활용할 타입을 명시
let str3 = generic('hello');  // 제네릭의 경우 타입 변수에 값을 유추 가능
// let str4 = generic<string>(100)   //error

generic<number>(123);   // 함수 호출 시 타입 변수에 활용할 타입을 명시
generic<boolean>(true);   // 함수 호출 시 타입 변수에 활용할 타입을 명시

//? +) 여러 제네릭 타입 변수 지정
// : 여러개의 독립적인 타입을 처리할 경우 사용
...
class BasicClass{
  ...
  constructor(value:string,addFunc:(s:string,y:string) => string) {
    this.value = value;
    this.add = addFunc;
...
let myGenericNumber = new GenricClass<number>(0,function(x,y){return x + y})
console.log(myGenericNumber.add(4,5));

let myGenericString = new GenricClass<string>('',function(x,y){return `${x} ${y}`});
console.log(myGenericString.add('이','승아'));
