...
// -content : 할 일의 내뇽을 문자열로 저장
// - completed : 할 일의 완료 상태를 나타내는 boolean데이터
...
// >> 'CRUD 프로그램의 기본 기능'
// Create(생성1) Read(읽기4) Update(수정2) Delete(삭제3)
...
function toggleTodo(id) {
  todos=todos.map((todo) =>{
    ...
    if(todo.id === id){
      ...
      // let array = [1,2,3];
      // let array2 =[...array]; >> array요소만 가져와소 새로운 배열을 생성(메모리 주소가 다름)
...
function displayTodos() {
  ...
  todos.forEach(todo => {
    // todo.completed는 boolena값
    //>>조건식 중에 연산자를 활용해서 '삼항연산자' (조건식 ? 참 : 거짓)
