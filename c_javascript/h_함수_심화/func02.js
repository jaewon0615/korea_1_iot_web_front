...
//* 배열의 콜백 함수의 형태
// 베열.메서드(콜백함수)
// >> 콜백함수를 가지는 배열의 메서드이 공통된 기능은 배열을 '순회'
...
//배열명.forEach(각 요소들에게 적용할 함수-콜백함수);
...
const newNubmers =numbers.map(function(value){
...
const evenNumbers =basicArray.filter(function (value){
  return value % 2 === 0;  //해당 조건이 true인 값만 새로운 배열에 저장
})
...
// 3.전체 배열의 요소 중 문자열의 길이가 4보다 크 요소만 변환-filter
// >>문자열데이터.length
...
// >>어떤 메서드(기능)이 반환(리턴)하는 값을 기반으로 또 다른 메서드(기능)을 줄줄이 사용하는것
