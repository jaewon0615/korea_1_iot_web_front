...
// :0부터 시작하는 인덱스 번호를 가잠(1씩 증가)

// :각각의 데이터를 '요소'라고 부름
// :크기가 고정되지 않고 프로그램 실행 중 동적으로 사용 가능
...
// 2) Array생성자를 사용 방식
// :새로운 배열 생성시 배열의 크기나 초기값을 지정하는 방법
...
//* 배열의 길이
// :배열명.length 속성
// >> 배열의 마지막 요소 인덱스 번호 = 배열의 길이 === 배열의 길이 -1
console.log(sports.length); //3

sports.length = 5;
console.log(sports[3]);  //undefiend-배열의 길이를 늘리면 새 요소는 undefiend로 초기화
console.log(sports);
...
// 1)indexOf()***
// :배열의 지정된 요소를 찾고 그 요소의 첫 번째 인덱스를 반환(없으면 -1반환)
...
// 2) 요소 삭제
// pop()******:배열의 마지막 요소를 제거,그 제거된 요소를 반환(빈 배열은 undefiend를 반환)
