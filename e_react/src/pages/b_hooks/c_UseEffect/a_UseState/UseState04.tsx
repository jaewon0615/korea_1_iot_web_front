...
? 요구 사항 정리
1) 폼 필드
: 사용자 아이디,비밀번호, 이메일 주소를 입력(문자열 데이터)
...
export default function UseState04() {
  ...
  //! 폼 입력 오류 메시지 상태 관리
  const [erros, setErros] = useState<IFormData>({
    id: "",
    password: "",
    email: "",
  });
  ...
  const handleSignUpSubmit = (e: React.FormEvent) => {
    ...
    // >> 최종 유효성 검사가 끝나면 해당 객체를 setErros에 전달
    ...
    if (!id) {
      // id입력 필드가 비워져있으면
      tempErros.id = "아이디를 입력해주세요"; // 오류 메시지 설정
      isValid = false;
    }
    ...
    if (!password) {
      // 비밀번호 입력 필드가 비워져있으면
      tempErros.password = "비밀번호를 입력해주세요"; // 오류 메시지 설정
      isValid = false;
    }
    ...
    if (!email) {
      // 이메일 입력 필드가 비워져있으면
      tempErros.email = "이메일을 입력해주세요"; // 오류 메시지 설정
      isValid = false;
    }
    ...
    >
      ...
      <form onSubmit={handleSignUpSubmit}>
        <div>
          ...
          {erros.id && <p style={{ color: "red" }}>{erros.id}</p>}
        </div>
        <div>
          <label>
            비밀번호:
