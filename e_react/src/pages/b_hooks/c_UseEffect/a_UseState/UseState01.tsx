...
/*
! useState
: React에서 제공하는 Hook중 하나
: 함수형 컴포넌트 내에서 상태를 관리하는 기능
  (컴포넌트 단위에서의 상태 관리)

: useState 훅 호출 시
  >> 변화되는 상태 값과 해당 상태값을 업데이트 하는 함수를 제공
  >> 상태 업데이트 함수는 비동기적으로 처리가 기본으로 진행
  >> 상태 변경 시 컴포넌트의 재렌더링을 유발
*/

export default function UseState01() {
//! useState의 기본 구문
// const [state,setState] = userState<type>(initialValue);
...
// >> Hook은 React 내부의 함수이기 때문에 import하여 사용
...
const handleUpClick =() => {
  ...
  // 2) 함수형 업데이트를 사용
  // >> 아전 상태 값을 기반으로 상태를 업데이트 하는 경우 (권장)
